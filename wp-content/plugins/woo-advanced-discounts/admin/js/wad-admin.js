(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on('focus', ".wad-taxonomies-selector", function () {
            // Store the current value on focus and on change
            let previous = $(this).val();

            if (typeof wad_tax_query_selected_option[$(this).attr('name')] === 'undefined') {
                let temp = new Array();
                temp[previous] = $(this).closest('tr').find('.wad-terms-selector').val();
                wad_tax_query_selected_option[$(this).attr('name')] = temp;
            } else {
                wad_tax_query_selected_option[$(this).attr('name')][previous] = $(this).closest('tr').find('.wad-terms-selector').val();
            }
        }).on("change", ".wad-taxonomies-selector", function (e) {
            let param = $(this).val();
            let wad_selected_option_key = wad_tax_query_selected_option[$(this).attr('name')][$(this).val()];

            $(this).closest('tr').find('.wad-terms-selector').html(wad_tax_query_recap[param]);
            if (wad_selected_option_key) {
                // mark elements previously selected.
                $(this).closest('tr').find('.wad-terms-selector option').each(function(i){
                    if (wad_selected_option_key.indexOf($(this).val()) >= 0) {
        ...
        $(document).on("click", ".add-rf-row", function () {
            // wad product list create page.
            if (wad_tax_query_recap) {
                let val = $(".wad-taxonomies-selector").last().val();
                $(".wad-terms-selector").last().html(wad_tax_query_recap[val]);
            }
        });

        // Handle taxonomies field when updating product list
        if (typeof wad_tax_query_recap !== 'undefined') {
            $(".wad-taxonomies-selector").each(function(index){
                let selectedEle = [];
                // Retrieve selected elements.
                $('.wad-terms-selector:eq( ' + index + ' ) :selected').each(function(i){
                    selectedEle.push($(this).val());
                });

                // update taxonomies choices.
                let val = $(this).val();
                $('.wad-terms-selector:eq( ' + index + ' )').html(wad_tax_query_recap[val]);

                // mark elements previously selected.
                $('.wad-terms-selector:eq( ' + index + ' ) option').each(function(i){
                    if (selectedEle.indexOf($(this).val()) >= 0) {
                        $(this).attr('selected','selected');
                    }
                });
                let temp = new Array();
                temp[val] = $('.wad-terms-selector:eq( ' + index + ' )').val();
                wad_tax_query_selected_option[$(this).attr('name')] = temp;
            });
        }
    });
