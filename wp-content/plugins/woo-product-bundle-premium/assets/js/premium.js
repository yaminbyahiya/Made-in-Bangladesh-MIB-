...
(function($) {
  $(document).on('click touch', '#wpc_add_update_key', function(e) {
    ...
    if (key.length > 10) {
      $this.val('Verifying...');
      $('.wpclever_update_keys_form').
          addClass('wpclever_update_keys_disabled');

      var data = {
        action: 'wpc_check_update_key', key: key,
      };
  ...
  $(document).on('click touch', '.wpc_remove_key', function(e) {
    ...
    if (r == true) {
      ...
      var data = {
        action: 'wpc_remove_key', key: $this.attr('data-key'),
      };
  ...
  $(document).on('click touch', '.wpc-notice .notice-dismiss', function(e) {
    ...
    if (key !== '') {
      ...
      var data = {
        action: 'wpc_dismiss_notice', key: key,
      };
