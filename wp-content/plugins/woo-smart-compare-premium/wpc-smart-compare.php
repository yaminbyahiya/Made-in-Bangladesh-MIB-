...
! defined( 'WOOSC_VERSION' ) && define( 'WOOSC_VERSION', '5.3.4' );
		...
		if ( ! class_exists( 'WPCleverWoosc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosc {
				...
				function __construct() {
					...
					// wpml
					add_filter( 'wcml_multi_currency_ajax_actions', array( $this, 'wcml_multi_currency' ), 99 );
				}
				...
				function init() {
					...
					add_shortcode( 'woosc_quick_table', array( $this, 'shortcode_quick_table' ) );
				...
				function enqueue_scripts() {
					...
					if ( self::get_setting( 'button_icon', 'no' ) !== 'no' ) {
						wp_enqueue_style( 'woosc-icons', WOOSC_URI . 'assets/css/icons.css', array(), WOOSC_VERSION );
					}
				...
				function admin_enqueue_scripts( $hook ) {
					...
					if ( strpos( $hook, 'woosc' ) ) {
						...
						wp_enqueue_script( 'fonticonpicker', WOOSC_URI . 'assets/libs/fonticonpicker/js/jquery.fonticonpicker.min.js', array( 'jquery' ) );
						wp_enqueue_style( 'woosc-icons', WOOSC_URI . 'assets/css/icons.css', array(), WOOSC_VERSION );
                                ...
                                <form method="post" action="options.php">
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <textarea name="woosc_settings[custom_attributes]" rows="10" cols="50"
                                                          class="large-text"><?php echo self::get_setting( 'custom_attributes' ); ?></textarea>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <textarea name="woosc_settings[custom_fields]" rows="10" cols="50"
                                                          class="large-text"><?php echo self::get_setting( 'custom_fields' ); ?></textarea>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="checkbox"
                                                       name="woosc_settings[open_bar_immediately]"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span class="description">Enable this to display the quick comparison table with related products on single product pages. You can customize the list of related products using our <a
                                                            ...
                                                            class="thickbox"
                                                            title="Install WPC Custom Related Products">WPC Custom Related Products</a> plugin.</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><?php esc_html_e( 'Position', 'woo-smart-compare' ); ?></th>
				...
				function get_table( $ajax = true, $products = null, $context = '' ) {
					...
					if ( is_array( $products ) && ( count( $products ) > 0 ) ) {
						...
						foreach ( $products as $product_id ) {
							...
							foreach ( $saved_fields as $saved_field ) {
								switch ( $saved_field ) {
									case 'image':
										if ( $link !== 'no' ) {
											$products_data[ $product_id ]['image'] = apply_filters( 'woosc_product_image', '<a ' . ( $link === 'yes_popup' ? 'class="woosq-link" data-id="' . $product_id . '" data-context="woosc"' : '' ) . ' href="' . $product_obj->get_permalink() . '" draggable="false" ' . ( $link === 'yes_blank' ? 'target="_blank"' : '' ) . '>' . $product_obj->get_image( self::get_setting( 'image_size', 'woosc-large' ), array( 'draggable' => 'false' ) ) . '</a>', $product_obj );
										} else {
											$products_data[ $product_id ]['image'] = apply_filters( 'woosc_product_image', $product_obj->get_image( self::get_setting( 'image_size', 'woosc-large' ), array( 'draggable' => 'false' ) ), $product_obj );
										}
									...
									case 'availability':
										...
										break;
									case 'attributes':
										$attributes = apply_filters( 'woosc_attributes', self::get_setting( 'attributes' ) );
									...
									case 'custom_attributes':
										$custom_attributes = explode( ',', apply_filters( 'woosc_custom_attributes', self::get_setting( 'custom_attributes' ) ) );
										...
										break;
									case 'custom_fields':
										if ( ( $custom_fields = apply_filters( 'woosc_custom_fields', self::get_setting( 'custom_fields' ) ) ) && ! empty( $custom_fields ) ) {
						...
						foreach ( $saved_fields as $saved_field ) {
							...
							switch ( $saved_field ) {
								case 'attributes':
									$saved_attributes = apply_filters( 'woosc_attributes', self::get_setting( 'attributes' ) );
									...
									break;
								case 'custom_attributes':
									$saved_custom_attributes = explode( ',', apply_filters( 'woosc_custom_attributes', self::get_setting( 'custom_attributes' ) ) );
									...
									break;
								case 'custom_fields':
									$saved_custom_fields = explode( ',', apply_filters( 'woosc_custom_fields', self::get_setting( 'custom_fields' ) ) );
				...
				function nav_menu_items( $items, $args ) {
					$selected    = false;
					$saved_menus = self::get_setting( 'menus', array() );
				...
				function shortcode_quick_table() {
                    ...
                    <section
                        ...
                        <div class="woosc-quick-table-products">
							<?php echo self::get_table( false, $related, 'quick_table' ); ?>
                        </div>
