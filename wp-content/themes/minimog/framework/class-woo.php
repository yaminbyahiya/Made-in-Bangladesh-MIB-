...
if ( ! class_exists( 'Minimog_Woo' ) ) {
	class Minimog_Woo {
		...
		public $best_selling_products = null;
		...
		public function initialize() {
			...
			add_filter( 'woocommerce_get_price_html', [ $this, 'simple_product_price_html' ], 999, 2 );
			add_filter( 'woocommerce_variation_sale_price_html', [ $this, 'product_price_html' ], 999, 2 );
			add_filter( 'woocommerce_variation_price_html', [ $this, 'product_price_html' ], 999, 2 );
		...
		public function custom_price_html( $price_amt, $regular_price, $sale_price ) {
			$html_price = '';
		...
		public function simple_product_price_html( $price, $product ) {
			if ( $product->is_type( 'simple' ) && $product->is_on_sale() ) {
				...
				if ( ! empty( $regular_price ) ) {
					$html_price = $this->custom_price_html( $price_amt, $regular_price, $sale_price );
				} else {
					$html_price = $price;
				}
			...
			if ( strpos( $html_price, 'class="price"' ) === false ) {
				$html_price = '<div class="price">' . $html_price . '</div>';
			}
		...
		public function product_minmax_price_html( $price, $product ) {
			...
			if ( ( $variation_min_price == $variation_min_regular_price ) && ( $variation_max_price == $variation_max_regular_price ) ) {
				...
				if ( $variation_min_regular_price === $variation_max_regular_price ) {
					$del_price = wc_price( $variation_max_regular_price );
				} else {
					$del_price = wc_format_price_range( $variation_min_regular_price, $variation_max_regular_price );
				}

				if ( $variation_min_price === $variation_max_price ) {
					$ins_price = wc_price( $variation_max_price );
				} else {
					$ins_price = wc_format_price_range( $variation_min_price, $variation_max_price );
				}

				$html_min_max_price = '<del>' . $del_price . '</del>' . '<ins>' . $ins_price . '</ins>';
			}
		...
		public function cart_totals_coupon_html( $coupon ) {
			...
			$coupon_html          = $discount_amount_html . sprintf(
					'<a href="%1$s" class="fly-cart-coupon remove-coupon-link" data-coupon="%2$s">%3$s</a>',
					esc_url( add_query_arg( 'remove_coupon', rawurlencode( $coupon->get_code() ), \Automattic\Jetpack\Constants::is_defined( 'WOOCOMMERCE_CHECKOUT' ) ? wc_get_checkout_url() : wc_get_cart_url() ) ),
					esc_attr( $coupon->get_code() ),
					'<span class="coupon-icon fal fa-trash-alt"></span>' . __( 'Remove', 'minimog' )
				);
		...
		public function get_variation_attributes_for_dropdown( $product, $variation ) {
			...
			foreach ( $empty as $empty_attr ) {
				...
				if ( empty( $taxonomy_info ) ) {
					$taxonomy_name = ucwords( $empty_attr );
				} else {
					$taxonomy_name = $taxonomy_info->name;
				}
		...
		public function get_product_ids_best_selling() {
			if ( null === $this->best_selling_products ) {
				$good_selling_products       = $this->get_product_ids_good_selling();
				$number                      = intval( Minimog::setting( 'shop_best_selling_list_number' ) );
				$this->best_selling_products = array_slice( $good_selling_products, 0, $number );
			}

			return $this->best_selling_products;
		}

		public function get_product_ids_good_selling() {
			$transient_name = 'minimog-product-ids-good-selling';
		...
		public function get_product_ids_new_arrivals() {
			$transient_name = 'minimog-product-ids-new-arrivals';
		...
		public function get_loop_product_image_size( $width = 450 ) {
		...
		public function get_product_image_slider_slide_html( $attachment_ids, $args = array() ) {
			...
			foreach ( $attachment_ids as $attachment_id ) {
				...
				$output['thumb_slides_html'] .= sprintf( '<div class="%1$s"><div class="swiper-thumbnail-wrap">%2$s%3$s</div></div>',
